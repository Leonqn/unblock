...
```
Запрос клиента → Кеш → Блокировка рекламы → Unblock → Retry → Маршрутизация DNS → Upstream
```
...
3. Если домен заблокирован — возвращает клиенту ошибку (SERVFAIL), и запрос не проходит
